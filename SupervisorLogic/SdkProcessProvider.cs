...
{
    public class SdkProcessProvider : IProcessProvider
    {
        ...
        public SdkProcessProvider(IServicesTable servicesTable)
        {
        ...
        public ProcessInfo StartProcces(string path)
        {
            ...
            return new ProcessInfo(process.ProcessName, process.Id, process.PagedMemorySize64);
        }
        ...
        private void MonitoreConfigureServices(object obj, ElapsedEventArgs e)
        {
            ...
            foreach (var service in _servicesTable.GetServices())
            {
                if (runningProcesses.Select(p => p.ProcessName).Contains(service.ServiceName) && !service.IsStarted)
                {
                    ...
                    ConfiguredServiceStarted?.Invoke(service); // Буду засовувати свій процес і відправляти по веб сокету.
                }
